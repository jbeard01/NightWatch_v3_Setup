...
const BaseReporter = require('nightwatch/lib/reporter/base-reporter.js');
const {Logger} = Utils;


// it's a copy from node_modules/nightwatch/lib/reporter/reporters/html.js to customize the report in a way we heed it
// The biggest change for now it fixMessageDash
module.exports = class HtmlReporter extends BaseReporter {
  ...
  getFolderPrefix() {
    let folderPrefix = 'nightwatch-html-report';
  ...
  fixMessageDash(module) {
    // HtmlReact UI splits message by dash, so we need to replace it with html entity to get full message
    if (module?.lastError?.message !== undefined) {
      module.lastError.message = module.lastError.message.replace(/-/g, '&#x2D;');
    }

    _.keys(module.completedSections).forEach((testCase) => {
      const commands = module.completedSections[testCase].commands;
      for (const cmd of commands) {
        if (cmd?.result?.message === undefined) continue;
        cmd.result.message = cmd.result.message.replace(/-/g, '&#x2D;');
      }
    });
  }
  ...
  async writeReport() {
    ...
    const destFolder = path.join(output_folder, this.getFolderPrefix());
    const filename = path.join(destFolder, 'index.html');
    const jsonString = JSON.stringify(results);
    HtmlReact.writeNightwatchHTMLReport(destFolder, 'index.html', jsonString);
    Logger.logDetailedMessage(
      Logger.colors.stack_trace(` Wrote HTML report file to: ${path.resolve(filename)}` + '\n'),
      'info'
    );
    if (shouldOpenReport) {
